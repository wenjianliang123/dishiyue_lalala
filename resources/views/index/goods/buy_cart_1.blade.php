...
//            alert('这是总价'+total_price);
            ...
            var shuliang=[];
            $('.box:checked').each(function(){
                var old_shuliang=$(this).attr('shuliang');
                shuliang.push(old_shuliang);
            });
            var shuliang_new=shuliang.join(',');
            if (shuliang_new==''){
                alert('请至少选择一件商品进行结算');
                return false;
            }
...
//return;
            $.ajax({
                ...
                url:"{{url('/index/order')}}",
                type:'post',
                data:{goods_id:_id,shuliang:shuliang,total_price:total_price},
                success:function(res){
                    ...
                    if(msg.code==1) {
                        ...
                        if(result){
                            location.href="{{url('/index/do_order')}}";
                        }else{
                            history.go(0);
                        }
